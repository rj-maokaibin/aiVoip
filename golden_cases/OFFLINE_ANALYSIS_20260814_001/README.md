...
`PCAP -> Packet/PCM/Media Analyzer -> CandidateDecision -> Call Reconstruction -> Cross-Layer -> Finding -> Artifact semantics -> Canonical Report -> Deterministic Diagnosis -> Truth Diff`
...
```bash
python tools/offline_analysis_golden_replay.py \
  --pcap '/data/voip-golden/tcpdump-2026-08-14(2).pcap' \
  --require-fixture
```
...
- SIP Call：1 通；Call-ID=`00ad1c804c33b255@192.168.3.200`；目标号码 `601`；双向 RTP。
...
- Canonical Report 必须显示 `OFFLINE_IMPORTED + CALL-001 + 601`，不得再出现 Packet 已重建 Call 但报告 `Call=None` 的矛盾。

完整 Truth 以 `manifest.yaml` 为机器权威。

## Answer Leakage 禁止

`manifest.yaml.expected` 只能在 Analyzer 全部执行完后由 `validate_offline_analysis_bundle()` 使用。禁止把 Golden 预期值、异常时间窗、号码、Frame/Seq 或根因标签注入生产 Analyzer、FindingComposer、Diagnosis、AI Prompt 或阈值选择逻辑。
