...
def _env_map(info: dict) -> dict[str, str]:
    ...
    for row in info.get("Config", {}).get("Env") or []:
        text = str(row)
        if "=" in text:
            key, value = text.split("=", 1); result[key] = value
    return result
...
def _compose_project(info: dict) -> str:
    return str(_labels(info).get("com.docker.compose.project") or "").strip()
...
def _is_transient_postgres_candidate(info: dict) -> bool:
    name = str(info.get("Name") or "").lstrip("/").lower()
    service = str(_labels(info).get("com.docker.compose.service") or "").lower()
    return name.startswith("voip-ai-gate-pg-") or service in {"gate-postgres", "test-postgres"}


def _postgres_score(info: dict, hostname: str) -> int:
    labels = _labels(info); env = _env_map(info)
    image = str(info.get("Config", {}).get("Image") or "").lower()
    service = str(labels.get("com.docker.compose.service") or "").lower()
    name = str(info.get("Name") or "").lstrip("/").lower()
    aliases: set[str] = set()
    for network in _network_rows(info): aliases.update(_network_aliases(info, network))
    score = 0
    if service == hostname.lower(): score += 30
    if hostname.lower() in aliases: score += 25
    if service in {"postgres", "postgresql", "db", "database"}: score += 15
    if "postgres" in image: score += 8
    if "postgres" in name: score += 5
    if env.get("POSTGRES_DB"): score += 3
    if env.get("POSTGRES_USER"): score += 2
    return score
...
def _resolve_hostname_inside_backend(backend_id: str, hostname: str) -> str:
    ...
    try: value = _capture(["docker", "exec", backend_id, "python", "-c", script, hostname])
...
def _discover_postgres_route(primary_network: str, backend_info: dict) -> dict:
    ...
    try: hostname = str(urlparse(database_url).hostname or "")
    except Exception: hostname = ""
    backend_project = _compose_project(backend_info)
    route = {
        ...
        "excluded_transient_count": 0,
        "additional_networks": [],
        "host_overrides": [],
    }
    if not hostname: return route
    ...
    try: ids = _capture(["docker", "ps", "-aq"]).split()
    except Exception: ids = []
    candidates: list[dict] = []
    for cid in ids:
        if cid == backend_id or backend_id.startswith(cid) or cid.startswith(backend_id): continue
        try: info = _docker_inspect(cid)
        except Exception: continue
        if _is_transient_postgres_candidate(info):
    ...
    trusted_candidates = [row for row in candidates if row["trusted"]]
    route["trusted_candidate_count"] = len(trusted_candidates)
    route["external_candidate_count"] = len(candidates) - len(trusted_candidates)
    ...
    top_score = running[0]["score"]; top = [row for row in running if row["score"] == top_score]
    if len(top) != 1:
    ...
    if not address:
        route["status"] = "candidate_no_address"; return route
    if hostname.lower() not in _network_aliases(candidate["info"], target_network):
        route["host_overrides"] = [{"hostname": hostname, "address": address, "container_id": candidate["id"], "container_name": candidate["name"]}]
...
def _discover_real_backend(feishu_secret_file: Path) -> tuple[dict, str, str, list[dict[str, str]]]:
    ids = _capture(["docker", "ps", "--filter", "label=com.docker.compose.service=backend", "-q"]).split(); matches=[]
    for cid in ids:
        info = _docker_inspect(cid); env = set(info.get("Config", {}).get("Env") or [])
        if "REPRODUCTION_PLATFORM_MODE=real" in env and "FEISHU_LIVE_ENABLED=true" in env: matches.append((cid, info))
    if len(matches) != 1: raise RuntimeError(f"EXPECTED_ONE_REAL_FEISHU_BACKEND_FOUND_{len(matches)}")
    _, info = matches[0]; networks = list(_network_rows(info).keys())
    if not networks: raise RuntimeError("REAL_BACKEND_HAS_NO_DOCKER_NETWORK")
    network = networks[0]; base_image = str(info.get("Config", {}).get("Image") or "").strip()
    if not base_image: raise RuntimeError("REAL_BACKEND_IMAGE_MISSING")
    _docker_inspect(base_image, image=True); mounts=[]; destinations=set()
    for mount in info.get("Mounts") or []:
        destination=str(mount.get("Destination") or ""); source=str(mount.get("Source") or "")
        if destination.startswith("/run/secrets/") and source: mounts.append({"source":source,"destination":destination}); destinations.add(destination)
    if "/run/secrets/feishu_app_secret" not in destinations:
        if not feishu_secret_file.is_file(): raise RuntimeError("FEISHU_SECRET_FALLBACK_MISSING")
...
def _runtime_tag(contract: dict, fingerprint: str) -> str:
    version = re.sub(r"[^a-zA-Z0-9_.-]+", "-", str(contract.get("runtime_version") or "v1")); return f"voip-live-acceptance:{version}-{fingerprint[:16]}"


def _prepare(args: argparse.Namespace) -> int:
    contract=_load_contract(args.contract.resolve()); backend_info,network,base_image,secret_mounts=_discover_real_backend(args.feishu_secret_file.resolve())
    database_route=_discover_postgres_route(network,backend_info); base_image_info=_docker_inspect(base_image,image=True); base_image_id=str(base_image_info.get("Id") or "")
    ...
    try:
        runtime_info=_docker_inspect(runtime_image,image=True); labels=runtime_info.get("Config",{}).get("Labels") or {}
...
def _run_in_runtime(args:argparse.Namespace)->int:
    ...
    backend_id=str(context.get("backend_container_id") or ""); backend_info=_docker_inspect(backend_id)
    ...
    runtime_env_path=None; container_name=f"voip-live-acceptance-{os.getpid()}-{uuid.uuid4().hex[:8]}"; created=False
    try:
        ...
        docker_args=["docker","create","--name",container_name,"--network",str(context["docker_network"]),"--env-file",str(args.env_file.resolve()),"--env-file",str(runtime_env_path),"-v",f"{workspace}:/workspace","-w","/workspace","-e","PYTHONPATH=/workspace/backend:/workspace","-e","MPLCONFIGDIR=/tmp/voip-live-acceptance-matplotlib","-e",f"LIVE_ACCEPTANCE_RUNTIME_CONTRACT={context['runtime_contract']}","-e",f"LIVE_ACCEPTANCE_RUNTIME_VERSION={context['runtime_version']}","-e",f"LIVE_ACCEPTANCE_ORCHESTRATOR_VERSION={context.get('orchestrator_version') or ORCHESTRATOR_VERSION}","-e",f"LIVE_ACCEPTANCE_RUNTIME_FINGERPRINT={context['runtime_fingerprint']}","-e",f"LIVE_ACCEPTANCE_SOURCE_REVISION={context['source_revision']}","-e",f"LIVE_ACCEPTANCE_WORKSPACE_REVISION={workspace_revision}","-e",f"LIVE_ACCEPTANCE_DATABASE_ROUTE_STATUS={(context.get('database_route') or {}).get('status','')}"]
        ...
        for item in args.set_env or []:
            ...
            docker_args += ["-e",item]
        docker_args += [str(context["runtime_image"]),*command]; _capture(docker_args); created=True
        for network in context.get("additional_networks") or []:
            if network and network!=context.get("docker_network"): _run(["docker","network","connect",str(network),container_name])
        _run(["docker","start","-a",container_name],check=False); state=_docker_inspect(container_name).get("State") or {}; return int(state.get("ExitCode") or 0)
    finally:
        if created: _run(["docker","rm","-f",container_name],check=False)
