...
class PacketIntelligenceEngine:
    ...
    def _stream_overlaps_call(self, stream: dict, call: dict, tolerance_seconds: float | None = None) -> bool:
        ...
        if start is None: start=call.get("start_time")
        if end is None: end=call.get("end_time")
        ss=stream.get("start_time"); se=stream.get("end_time")
    ...
    def _attach_media_direction_health(self, calls: list[dict], streams: list[dict]) -> None:
        ...
        for call in calls:
            ...
            if not om or not am:
                ...
                continue
            # Only sendrecv/sendrecv is treated as requiring both directions in V1.
        ...
        for stream in streams:
            for event in stream["events"]:
                anomalies.append({
                    ...
                    "evidence": {"stream_id": stream["stream_id"], **event["details"]},
                })
