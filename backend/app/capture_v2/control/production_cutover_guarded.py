...
class CutoverStageError(RuntimeError):
...
keys=set(os.environ["SAFE_KEYS"].split(","))
out={}
...
def _release_preflight(repo_root: Path) -> tuple[int, dict[str, Any], str]:
    ...
    try:
        cp = _sudo(
            ...
            "--env-file", str(PRODUCTION_ENV), "--mode", "release", "--out", str(out),
...
def _refresh_source_manifest(repo_root: Path) -> tuple[bytes, dict[str, Any]]:
    path = repo_root / "release/source_manifest.json"
    original = path.read_bytes()
    update = _run([sys.executable, "tools/source_manifest_gate.py", "--update"], cwd=repo_root, timeout=180)
    if update.returncode != 0:
        raise CutoverStageError("SOURCE_MANIFEST_REFRESH_FAILED", _safe_tail(update.stderr or update.stdout))
    verify = _run([sys.executable, "tools/source_manifest_gate.py"], cwd=repo_root, timeout=180)
    if verify.returncode != 0:
        raise CutoverStageError("SOURCE_MANIFEST_VERIFY_FAILED", _safe_tail(verify.stderr or verify.stdout))
    try:
        report = json.loads(verify.stdout)
    except Exception as exc:
        raise CutoverStageError("SOURCE_MANIFEST_REPORT_INVALID", type(exc).__name__) from exc
    return original, report


def _restore_source_manifest(repo_root: Path, original: bytes | None) -> None:
    if original is not None:
        (repo_root / "release/source_manifest.json").write_bytes(original)


def _deployment(repo_root: Path) -> subprocess.CompletedProcess[str]:
    return _sudo(
        str(repo_root / "deploy/voip-ai"), "--env", str(PRODUCTION_ENV), "--project", PROJECT, "deploy",
        cwd=repo_root, timeout=5400,
    )


def _runtime_verify(repo_root: Path) -> subprocess.CompletedProcess[str]:
    return _sudo(
        str(repo_root / "deploy/voip-ai"), "--env", str(PRODUCTION_ENV), "--project", PROJECT, "verify",
...
def _backend_authority(repo_root: Path) -> tuple[int, dict[str, Any], str]:
    code = (
        "import json; from app.capture_v2.runtime import capture_authority_mode; "
    ...
    cp = _sudo(*_compose_args(PRODUCTION_ENV, "exec", "-T", "backend", "python", "-c", code), cwd=repo_root, timeout=90)
    if cp.returncode != 0:
        return cp.returncode, {}, _safe_tail(cp.stderr or cp.stdout)
    try:
...
def _rollback(repo_root: Path, *, backup_path: Path, pre_running: list[str]) -> dict[str, Any]:
    ...
    if err: result["env_restore_error"] = err
    if not ok: return result

    if not pre_running:
        down = _sudo(*_compose_args(PRODUCTION_ENV, "down", "--remove-orphans"), cwd=repo_root, timeout=600)
        result.update({
            "rollback_command": "compose down --remove-orphans",
            "rollback_return_code": down.returncode,
            "runtime_restored": down.returncode == 0,
        })
        if down.returncode != 0: result["rollback_error"] = _safe_tail(down.stderr or down.stdout)
        return result

    up = _sudo(*_compose_args(PRODUCTION_ENV, "up", "-d", "--force-recreate", *pre_running), cwd=repo_root, timeout=1800)
    result.update({
        "rollback_command": "compose up --force-recreate <pre-running-services>",
        "rollback_return_code": up.returncode,
        "runtime_restored": up.returncode == 0,
    })
    if up.returncode != 0: result["rollback_error"] = _safe_tail(up.stderr or up.stdout)
    return result
...
def run(*, repo_root: Path, authorization_path: Path) -> tuple[int, dict[str, Any]]:
    ...
    if not ok: return 1, {**payload, "verdict": "FAIL", "reason": reason}
    ...
    ok, reason, _ = _validate_release_gate(repo_root)
    if not ok: return 1, {**payload, "verdict": "FAIL", "reason": reason}
    ...
    mutated = False
    success = False
    source_manifest_original: bytes | None = None
    failure_reason: str | None = None
    failure_detail = ""

    try:
        source_manifest_original, manifest_report = _refresh_source_manifest(repo_root)
        payload["stages"]["source_manifest"] = {
            "passed": True,
            "file_count": manifest_report.get("file_count"),
            "aggregate_sha256": manifest_report.get("aggregate_sha256"),
        }

        env_rc, env_change, env_err = _write_cutover_env(repo_root, backup_path)
        payload["stages"]["env_switch"] = {
            "return_code": env_rc,
            "backup_path": str(backup_path),
            "overrides": env_change.get("overrides") if env_change else None,
        }
        if env_rc != 0:
            raise CutoverStageError("PRODUCTION_ENV_SWITCH_FAILED", env_err)
        mutated = True
        ...
        if release_rc != 0 or release_report.get("release_status") != "PASS":
            raise CutoverStageError("POST_SWITCH_RELEASE_PREFLIGHT_FAILED", release_err)
        ...
        if deploy.returncode != 0:
            raise CutoverStageError("PRODUCTION_DEPLOY_FAILED")
        ...
        if verify.returncode != 0:
            raise CutoverStageError("PRODUCTION_RUNTIME_VERIFY_FAILED")
        ...
        if auth_rc != 0:
            raise CutoverStageError("PRODUCTION_AUTHORITY_PROBE_FAILED", auth_err)
        if not (
            ...
            raise CutoverStageError("PRODUCTION_V2_AUTHORITY_NOT_ESTABLISHED")

        running_rc, running, running_err = _running_services(repo_root)
        payload["post_running_services"] = running
        missing = sorted(set(REQUIRED_APP_SERVICES) - set(running))
        payload["stages"]["required_services_running"] = {"passed": running_rc == 0 and not missing, "missing": missing}
        if running_rc != 0 or missing:
            raise CutoverStageError("PRODUCTION_REQUIRED_SERVICES_NOT_RUNNING", running_err)

        success = True
    except CutoverStageError as exc:
        failure_reason, failure_detail = exc.reason, exc.detail
    except subprocess.TimeoutExpired as exc:
        failure_reason, failure_detail = "PRODUCTION_CUTOVER_TIMEOUT", str(exc.cmd)
    except Exception as exc:
        failure_reason, failure_detail = "PRODUCTION_CUTOVER_EXCEPTION", f"{type(exc).__name__}:{_safe_tail(str(exc))}"
    finally:
        if mutated and not success:
            payload["rollback"] = _rollback(repo_root, backup_path=backup_path, pre_running=pre_running)
        _restore_source_manifest(repo_root, source_manifest_original)

    if not success:
        payload["verdict"] = "FAIL"
        payload["reason"] = failure_reason or "PRODUCTION_CUTOVER_FAILED"
        if failure_detail: payload["error"] = failure_detail
        if mutated and not (payload.get("rollback") or {}).get("runtime_restored"):
            payload["reason"] = "PRODUCTION_CUTOVER_FAILED_ROLLBACK_UNVERIFIED"
        return 1, payload
