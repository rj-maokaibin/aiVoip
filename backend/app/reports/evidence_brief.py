...
def build_completeness(*, evidences: list[dict], analyzer_states: dict[str, dict], scope_type: str,
                       results: dict[str, dict | None] | None = None) -> dict:
    results = results or {}
    def state(name: str) -> dict:
        item = analyzer_states.get(name) or {}; status = str(item.get("status") or "UNAVAILABLE")
        return {"status": status, "available": status in {"SUCCESS", "PARTIAL_SUCCESS"},
    ...
    return {"state": state_value, "scope_type": scope_type, "capture": capture, "analyzers": analyzers,
            ...
            "boundary": "证据完整，可进行当前范围的 Packet/PCM/Media 初步证据分析。" if state_value == "COMPLETE"
...
def build_packet_summary(packet: dict | None) -> dict:
    ...
    return {"available": True, "packet_count": summary.get("packet_count"), "sip_message_count": summary.get("sip_message_count"),
            "call_count": summary.get("call_count"), "rtp_stream_count": summary.get("rtp_stream_count"),
            "rtcp_report_count": summary.get("rtcp_report_count"), "streams": streams, "calls": packet.get("calls", [])}
...
def build_pcm_summary(pcm: dict | None) -> dict:
    ...
    for stream in pcm.get("streams", []) or []:
        ...
        for session in stream.get("sessions", []) or []:
            ...
            sessions.append({"session_index": session.get("session_index"), "start_time": session.get("start_time"),
                             "end_time": session.get("end_time"), "packet_count": session.get("packet_count"),
                             "audio_duration_seconds": session.get("audio_duration_seconds"), "gap_event_count": session.get("gap_event_count"),
                             "rms_dbfs": signal.get("rms_dbfs", signal.get("dbfs")), "peak_dbfs": signal.get("peak_dbfs"),
                             "peak_amplitude": signal.get("peak"), "dc_offset": signal.get("dc_offset"),
                             "clipping_percent": signal.get("clipping_percent"), "hum": session.get("hum"),
                             "spectral": session.get("spectral"), "silence_event_count": len(session.get("silence_events", []) or []),
                             "click_pop_event_count": len(session.get("click_pop_events", []) or []), "dtmf_sequences": session.get("dtmf_sequences", [])})
...
def build_report_payload(*, case: dict, scope_type: str, scope_id: str, session: dict | None, call: dict | None,
    ...
    resolved_call = display_call if display_call is not None else call; context = analysis_context or {}
    semantic_issues = list(context.get("semantic_issues") or [])
    if semantic_issues:
        completeness["semantic_status"] = "INCOMPLETE"; completeness["semantic_issues"] = semantic_issues
        completeness["reviewability"] = "NOT_FULLY_REVIEWABLE"; completeness["state"] = "PARTIAL"
        ...
        completeness["semantic_status"] = "OK"; completeness["semantic_issues"] = []
    ...
    boundary = {"root_cause_authority": "PRELIMINARY_EVIDENCE_ONLY",
    ...
    payload = {"schema_version": REPORT_SCHEMA_VERSION, "composer_version": REPORT_COMPOSER_VERSION,
               "report_version": report_version, "generated_at": generated_at or utcnow_iso(),
               "scope": {"type": scope_type, "id": scope_id}, "case": case, "session": session, "call": resolved_call,
               "display_call": resolved_call, "analysis_context": context, "environment": environment or {},
               "environment_fingerprint": environment_fingerprint(environment), "headline": headline,
               "finding_count": len(findings), "highest_severity": highest, "completeness": completeness,
               "packet_summary": build_packet_summary(packet), "pcm_summary": build_pcm_summary(pcm),
               "media_summary": (media or {}).get("summary") if media else None, "findings": findings,
    ...
    payload["input_snapshot_hash"] = canonical_hash({"scope": payload["scope"], "case": case, "session": session,
        "display_call": resolved_call, "analysis_context": context, "environment": environment, "evidences": evidences,
        "analyzers": analyzer_states, "result_hashes": {key: canonical_hash(value) if value is not None else None for key, value in results.items()}})
    return payload
...
def _multi_call_html(payload: dict) -> str:
    ...
    rows = "".join(f"<tr><td>{_esc(x.get('title'))}</td><td>{_esc(x.get('occurrence_calls'))}/{_esc(x.get('total_calls'))}</td>"
                   f"<td>{_esc(round((x.get('reproduction_rate') or 0) * 100, 2))}%</td><td>{_esc(x.get('stability'))}</td></tr>" for x in groups)
...
def _ab_html(payload: dict) -> str:
    ...
    for comp in comparisons:
        rows = "".join(f"<tr><td>{_esc(x.get('title'))}</td><td>{_esc(round((x.get('environment_a_rate') or 0) * 100, 2))}%</td>"
                       f"<td>{_esc(round((x.get('environment_b_rate') or 0) * 100, 2))}%</td><td>{_esc(round((x.get('absolute_rate_delta') or 0) * 100, 2))}%</td>"
                       f"<td>{'是' if x.get('significant_by_v1_rule') else '否'}</td></tr>" for x in comp.get("differences", []) or [])
...
def _scope_line(card: dict) -> str:
    ...
    for label, key in (("层", "layer"), ("方向", "direction"), ("PCM Tap", "pcm_tap"), ("Call", "call_id"), ("RTP Stream", "rtp_stream_id"), ("SSRC", "ssrc")):
        if scope.get(key) not in (None, "", "UNKNOWN"): parts.append(f"{label}：{_esc(scope.get(key))}")
    return "｜".join(parts) or "范围：UNKNOWN（当前 Evidence 不足以绑定更细 Scope）"


def _time_line(card: dict) -> str:
    value = card.get("time") or {}; start = value.get("absolute_start_utc"); end = value.get("absolute_end_utc")
    if start is None and end is None: return "绝对时间：UNKNOWN（当前 Evidence 未提供可绑定时间）"
    ...
    semantic_cn = {"OBSERVATION_BOUNDARY": "观察边界（非精确异常区间）", "ANALYSIS_WINDOW": "分析窗口（非精确异常区间）",
                   "EVENT_OR_ANALYZER_RANGE": "Finding/Analyzer 时间范围", "UNKNOWN": "时间边界未知"}.get(semantics, semantics)
    exact = "是" if value.get("exact_event_window_known") else "否"
    return f"绝对时间（UTC）：{absolute}｜时间语义：{_esc(semantic_cn)}｜精确异常首末时刻：{exact}" + (f"｜Call 相对时间：{_esc(relative)}" if relative else "")
...
def _measurements_html(card: dict) -> str:
    ...
    for measurement in card.get("measurements") or []:
        ...
        rows.append(f"<div class='metric'><b>{_esc(measurement.get('label'))}</b><span>{_esc(measurement.get('value'))}{unit}</span>{meaning}</div>")
...
def _packet_refs_html(card: dict) -> str:
    ...
    if not refs: return ""
    rows = "".join(f"<tr><td>{_esc(item.get('event'))}</td><td>{_esc(item.get('previous_frame'))} → {_esc(item.get('current_frame'))}</td>"
                   f"<td>{_esc(item.get('previous_seq'))} → {_esc(item.get('current_seq'))}</td><td>{_esc(item.get('delta_ms'))}</td><td>{_esc(item.get('classification'))}</td></tr>" for item in refs)
    return "<div class='evidence-sub'><b>Packet / Frame 下钻</b><table><thead><tr><th>#</th><th>Frame</th><th>RTP Seq</th><th>Delta(ms)</th><th>语义</th></tr></thead>" + f"<tbody>{rows}</tbody></table></div>"
...
def _visuals_html(card: dict) -> str:
    ...
    for artifact in card.get("visual_evidence") or []:
        ...
        if url: parts.append(f"<figure><img loading='lazy' src='{url}' alt='{atype}｜{caption}'><figcaption>{atype}｜{caption}</figcaption></figure>")
        else: parts.append(f"<figure><figcaption>{atype}｜{caption}｜离线/Bundle 中可查看</figcaption></figure>")
...
def _audio_html(card: dict) -> str:
    ...
    if status != "AVAILABLE": return f"<div class='audio-unavailable'><b>异常音频：暂不可用</b><br>{_esc(audio.get('reason'))}</div>"
    clips = []
    for artifact in audio.get("clips") or []:
        url = artifact.get("content_url")
        player = f"<audio controls preload='none' src='{_esc(url)}'>浏览器不支持音频播放。</audio>" if url else "<div class='small'>离线报告：该音频已进入 Evidence Bundle，可从 Bundle 下载试听。</div>"
        clips.append(f"<div class='audio-clip'><div><b>{_esc(artifact.get('type'))}</b>｜{_esc(artifact.get('caption'))}</div>{player}</div>")
    return "<div class='evidence-sub'><b>可试听异常音频</b>" + "".join(clips) + "</div>"


def _action_html(card: dict) -> str:
    contract = card.get("action_contract") or {}; steps = "".join(f"<li>{_esc(step)}</li>" for step in (contract.get("execution_steps") or []))
    acceptance = card.get("verification_acceptance") or contract.get("acceptance_criteria")
    return f"<div class='next-action'><b>下一步建议：</b>{_esc(card.get('next_action') or '复核本 Finding 的原始 Evidence。')}" + (f"<ol>{steps}</ol>" if steps else "") + f"<div><b>验证通过标准：</b>{_esc(acceptance or '新增证据必须绑定本 Finding，并使证据边界发生可解释变化。')}</div></div>"
...
def _evidence_card_html(finding: dict) -> str:
    ...
    return (f"<section class='finding sev-{_esc(finding.get('severity', 'INFO')).lower()}' id='finding-{_esc(finding.get('finding_id') or finding.get('stable_key'))}'>"
            ...
            + "<h4>关键测量</h4>" + _measurements_html(card) + "<h4>关键可视化证据</h4>" + _visuals_html(card)
...
def _completeness_html(payload: dict) -> tuple[str, str, str]:
    comp = payload.get("completeness") or {}; frozen = payload.get("capture_quality") or comp.get("frozen_v1") or {}; dimensions = frozen.get("dimensions") or {}
    if dimensions:
        rows = []
        for name in ("PCAP", "SIP", "RTP", "PCM_RX", "PCM_TX", "DEBUG", "CORRELATION"):
            item = dimensions.get(name) or {}; available = bool(item.get("available")); requirement = "必需" if item.get("requirement") == "REQUIRED" else "可选"
            rows.append(f"<tr><td>{_esc(name)}</td><td>{'✅ 可用' if available else '⚠️ 缺失/不可用'}</td><td>{requirement}</td><td>{_esc(item.get('impact'))}</td></tr>")
    ...
    capture = comp.get("capture") or {}; rows = "".join(f"<tr><td>{_esc(key)}</td><td>{'✅ 可用' if value else '⚠️ 缺失/不可用'}</td></tr>" for key, value in capture.items())
...
def render_report_html(payload: dict) -> str:
    ...
    findings = payload.get("findings", []); cards = [_evidence_card_html(finding) for finding in findings]
    if not cards: cards = ["<div class='ok'>当前可用证据中未发现明显异常 Finding。</div>"]
    comp = payload.get("completeness") or {}; completeness_state, completeness_boundary, completeness_table = _completeness_html(payload)
    review = (comp.get("reviewability_contract") or {}).get("state") or comp.get("reviewability") or "UNKNOWN"
    normal = "".join(f"<li>✅ {_esc(item.get('text'))}</li>" for item in (payload.get("normal_evidence") or payload.get("normal_and_exclusion_evidence") or [])) or "<li>暂无可展示的排除性证据。</li>"
    packet = payload.get("packet_summary") or {}
    stream_rows = "".join(f"<tr><td>{_esc(stream.get('source'))}</td><td>{_esc(stream.get('destination'))}</td><td>{_esc(stream.get('packet_count'))}</td>"
                          f"<td>{_esc(stream.get('observed_packet_count'))}</td><td>{_esc(stream.get('duplicate_packets'))}</td><td>{_esc(stream.get('lost_packets'))}</td>"
                          f"<td>{_esc(stream.get('loss_rate'))}</td><td>{_esc(stream.get('p95_jitter_ms'))}</td><td>{_esc(stream.get('codec'))}</td><td>{_esc(stream.get('ptime_ms'))}</td></tr>" for stream in packet.get("streams", []))
    pcm_rows = []
    for stream in (payload.get("pcm_summary") or {}).get("streams", []) or []:
        tap = (stream.get("tap") or {}).get("name")
        for session in stream.get("sessions", []) or []:
            pcm_rows.append(f"<tr><td>{_esc(tap)}</td><td>{_esc(session.get('session_index'))}</td><td>{_esc(session.get('packet_count'))}</td>"
                            f"<td>{_esc(session.get('rms_dbfs'))}</td><td>{_esc(session.get('peak_dbfs'))}</td><td>{_esc((session.get('hum') or {}).get('dominant_family'))}</td><td>{_esc((session.get('hum') or {}).get('level'))}</td></tr>")
    artifact_list = "".join(f"<li>{_esc(artifact.get('type'))}｜{_esc(artifact.get('filename'))}｜SHA256 {_esc(str(artifact.get('sha256') or '')[:12])}…</li>" for artifact in payload.get("artifacts", []) or []) or "<li>暂无衍生 Artifact。</li>"
    ...
    if offline:
        call_line = ("<h3>4. 当前离线 Call 重建结果</h3>"
                     f"<p>分析方式：离线证据导入｜复现 Session：不适用｜重建 Call：{_esc(context.get('reconstructed_call_count'))}。</p>" +
                     (f"<p>Call：{_esc(call.get('id') or call.get('call_no'))}｜SIP Call-ID：{_esc(call.get('sip_call_id') or call.get('external_call_ref'))}｜状态：{_esc(call.get('status'))}｜开始：{_esc(call.get('started_at') or call.get('start_time'))}｜结束：{_esc(call.get('ended_at') or call.get('end_time'))}｜号码：{_esc(call.get('caller') or '?')} → {_esc(call.get('dialed_number') or '?')}</p>" if call else "<p>Call：UNKNOWN（离线 Evidence 无法唯一绑定诊断 Call）。</p>"))
        session_history = "当前为离线证据导入；系统不会为了填充报告而创建 ReproductionSession/ReproductionCall。"
    else:
        call_line = f"<p>Session：{_esc(session.get('id'))}｜Call：{_esc(call.get('call_no') or call.get('id'))}｜状态：{_esc(call.get('status'))}｜开始：{_esc(call.get('started_at'))}｜结束：{_esc(call.get('ended_at'))}</p>"
        session_history = "Canonical JSON 保留 scope/environment/version；Case 页面按最新优先展示历史 Session，不同 Environment Fingerprint 不直接混合统计。"
    card_summary = payload.get("evidence_card_summary") or {}; bundle = payload.get("evidence_bundle_summary") or {}; bundle_status = "已生成" if bundle.get("status") == "AVAILABLE" else "可通过 Bundle API 生成/下载"; canonical = payload.get("canonical_finalization") or {}
