...
from app.db.models import Evidence, Job
...
def _profile_path(profile_id: str) -> Path:
    ...
    path = (settings.profile_root / "pcm" / f"{profile_id}.yaml").resolve()
    root = (settings.profile_root / "pcm").resolve()
    if root not in path.parents:
        raise ValueError("PCM_PROFILE_PATH_INVALID")
    if not path.exists():
...
def analyze_pcm_evidence(self, job_id: str, evidence_id: str, profile_id: str):
    ...
    try:
        ...
        engine = PcmIntelligenceEngine(profile)
        ...
        transition_job(db, job, JobStatus.RUNNING, reason='pcm_analysis_started'); db.commit()
        suffix = Path(evidence.filename).suffix or ".pcap"
        with tempfile.TemporaryDirectory(prefix="voip-pcm-") as td:
            local = Path(td) / f"input{suffix}"
            materialize_evidence(evidence, local)
            result = engine.analyze_pcap(local)
            encoded = json.dumps(result, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
            result_key = f"cases/{job.case_id}/analysis/{run.id}/pcm_analysis.json"
        ...
        transition_job(db, job, JobStatus(final_status), reason='pcm_analysis_complete')
        ...
        if "job" in locals() and job:
            ...
            try: transition_job(db, job, JobStatus.FAILED, reason='pcm_analysis_failed')
