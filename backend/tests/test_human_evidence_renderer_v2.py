...
def test_human_spectrum_is_continuous_dbfs_measurement():
    png, meta = render_human_spectrum_png_from_wav(
        _tone_wav(),
        canonical_spectral={"peaks": [{"frequency_hz": 150.0, "energy_ratio": 0.2}, {"frequency_hz": 250.0, "energy_ratio": 0.1}]},
        reference_frequencies_hz=[50, 150, 250, 350],
        title="Periodic spectrum",
    )
    assert png.startswith(PNG)
    assert meta["measurement_method"] == "NUMPY_RFFT_HANN_COHERENT_GAIN_V1"
    assert meta["level_unit"] == "dBFS"
    assert meta["amplitude_reference"] == "PCM16_FULL_SCALE_32768"
...
def test_human_waveform_and_legacy_spectrogram_generate_png_without_new_diagnosis():
    waveform = {
        ...
        "bins": [
            {"t": 0.0, "min": -1000, "max": 1000, "rms_dbfs": -30.0},
            {"t": 0.5, "min": -15000, "max": 16000, "rms_dbfs": -12.0},
            {"t": 0.99, "min": -500, "max": 500, "rms_dbfs": -36.0},
        ],
    }
    spec = {
        "times": [0.0, 0.5, 1.0],
        "frequencies": [0.0, 500.0, 1000.0, 2000.0, 4000.0],
        "db": [
            [10.0, 20.0, 30.0, 10.0, 0.0],
            [12.0, 25.0, 35.0, 12.0, 1.0],
            [9.0, 19.0, 28.0, 9.0, 0.0],
        ],
    }
    wave_png = render_human_waveform_png(waveform, anomaly_start=0.45, anomaly_end=0.60)
    spectrogram_png = render_human_spectrogram_png(spec, anomaly_start=0.45, anomaly_end=0.60)
    assert wave_png.startswith(PNG)
...
def test_human_explanation_copies_canonical_finding_and_has_no_diagnostic_authority():
    ...
    assert value["observations"] == [finding.observation]
    assert value["meaning"] == finding.interpretation
    assert value["evidence_boundary"] == finding.root_cause_boundary
    assert value["source_authority"] == "CANONICAL_FINDING"
    assert value["diagnostic_authority"] == "NONE"
...
def _human_meta() -> dict:
    return {
        ...
        "human_explanation": {
            ...
            "observations": ["检测到150Hz和250Hz峰。"],
            "meaning": "支持周期性干扰证据。",
            "evidence_boundary": "不能单独确认物理根因。",
            "plain_language_summary": "存在周期性音频干扰。",
        },
...
def test_human_visual_replaces_same_type_machine_only_in_presentation_projection():
    machine_spectrum = _Artifact("m1", "SPECTRUM_PNG", {"renderer_version": "evidence-renderer-v2"})
    human_spectrum = _Artifact("h1", "SPECTRUM_PNG", _human_meta())
    machine_wave = _Artifact("m2", "WAVEFORM_PNG", {"renderer_version": "evidence-renderer-v2"})
...
def test_human_projection_caption_contains_plain_language_sections():
    human = _Artifact("h1", "SPECTRUM_PNG", _human_meta())
    meta = _projection_metadata(human)
    caption = meta["annotation_contract"]["caption"]
    assert "这张图怎么看" in caption
    assert "图中发现" in caption
