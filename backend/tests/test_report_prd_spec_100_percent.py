...
def _artifact(artifact_id: str, artifact_type: str, *, audio: bool = False) -> dict:
    ext = "wav" if audio else "png"
    return {
        "artifact_id": artifact_id,
        "type": artifact_type,
        "filename": f"{artifact_id}.{ext}",
        "content_type": "audio/wav" if audio else "image/png",
        "sha256": ("a" if not audio else "d") * 64,
        "size_bytes": 1234,
        "local_path": f"/tmp/{artifact_id}.{ext}",
        ...
        "metadata": {
            ...
            "annotation_contract": {"caption": artifact_type, "title": artifact_type} if not audio else {},
            "created_at": "2026-08-23T00:00:00+00:00",
        },
    }


def _bundle_artifact(artifact_id: str, artifact_type: str, filename: str, content_type: str, fill: str) -> dict:
    return {
        "artifact_id": artifact_id,
        "type": artifact_type,
        "filename": filename,
...
def _payload() -> dict:
    ...
    bundle = _bundle_artifact("bundle-1", "EVIDENCE_BUNDLE", "evidence-bundle-internal-full-v2.zip", "application/zip", "b")
    manifest = _bundle_artifact("manifest-1", "MANIFEST_JSON", "manifest.json", "application/json", "c")
    return {
        ...
        "analysis_context": {"analysis_mode": "OFFLINE_IMPORTED", "reconstructed_call_count": 1, "diagnostic_call_count": 1},
        "display_call": {
            "id": "CALL-001",
            "status": "TERMINATED",
            "caller": "8000",
        ...
        "packet_summary": {
            ...
            "rtp_stream_count": 2,
            "calls": [{"call_id": "sip-1"}],
            "streams": [{"stream_id": "rtp-up", "loss_rate": 0.0}, {"stream_id": "rtp-down", "loss_rate": 0.0}],
        },
        "pcm_summary": {
            "available": True,
            "streams": [
        ...
        "completeness": {
            ...
            "analyzers": {"packet": {"available": True}, "pcm": {"available": True}, "media": {"available": True}},
        },
        "findings": [{
            ...
            "artifact_refs": [{**spectrum, "role": "FINDING"}, {**waveform, "role": "FINDING"}, {**audio, "role": "FINDING"}],
            "correlation": {},
        }],
        "finding_count": 1,
        ...
        "diagnosis": {
            ...
            "plan": [
                {"action_type": "REQUEST_USER_EVIDENCE", "reason": "A/B区分具体硬件节点", "risk_level": "USER", "auto_execute": False, "params": {"purpose": "close_specific_hardware_root_cause"}, "priority": 40},
                {"action_type": "REQUEST_MULTI_POINT_PCAP", "reason": "定位RTP抖动产生区间", "risk_level": "USER", "auto_execute": False, "params": {"purpose": "locate_jitter_segment"}, "priority": 65},
            ],
        },
        "evidence_bundle_summary": {"status": "AVAILABLE", "profile": "INTERNAL_FULL", "manifest_schema": "evidence-bundle-v1"},
    }


def _report() -> SimpleNamespace:
    return SimpleNamespace(id="report-2", case_id="case-1", session_id=None, call_id="CALL-001", scope_type="CASE", scope_id="case-1", version=2, status="COMPOSING")
...
def _text(block: dict) -> str:
    for key in ("text", "heading1", "heading2", "heading3", "bullet"):
        body = block.get(key)
        if body:
            return "".join(str((item.get("text_run") or {}).get("content") or "") for item in body.get("elements") or [])
    return ""


def test_canonical_v2_finding_contract_is_self_contained() -> None:
    payload, report = _payload(), _report()
    finalize_report_contract(report, payload)
    assert payload["version"] == 2 and payload["status"] == "COMPLETE"
    ...
    assert dimensions["DEBUG"]["requirement"] == "OPTIONAL" and dimensions["DEBUG"]["available"] is False
    ...
    assert {item["type"] for item in card["visual_evidence"]} >= {"SPECTRUM_PNG", "WAVEFORM_PNG"}
...
def test_feishu_and_html_share_d112_and_no_legacy_truth_layer() -> None:
    ...
    blocks, _, _ = service._core_blocks(report, payload, history_blocks=[service._text("历史 V1 已归档为审计快照；V2 正文不混排旧结论。", 12)])
    text = "\n".join(_text(block) for block in blocks if _text(block))
    assert [text.index(title) for title in D112_ORDER] == sorted(text.index(title) for title in D112_ORDER)
    for forbidden in FORBIDDEN:
        assert forbidden not in text
    assert "A/B 硬件变量验证" in text and "多点 PCAP 链路定位" in text
    assert "REQUEST_USER_EVIDENCE" not in text and "risk=USER" not in text
    assert "验证步骤：基线A" in text and "B→A" in text
    assert "PCAP：可用｜必需" in text and "DEBUG：缺失/不可用｜可选" in text
    assert "Evidence Bundle：已生成" in text and "Manifest：已生成" in text

    rendered = render_report_html(payload)
    for forbidden in FORBIDDEN:
        assert forbidden not in rendered
    assert [rendered.index(title) for title in D112_ORDER] == sorted(rendered.index(title) for title in D112_ORDER)
    assert "观察边界（非精确异常区间）" in rendered
    assert "验证通过标准" in rendered and "B→A" in rendered
...
def test_root_cause_authority_is_not_upgraded_by_v2_projection() -> None:
    ...
    finalize_report_contract(report, payload)
    finding = payload["findings"][0]
    text = " ".join((str(finding.get("observation") or ""), str(finding.get("interpretation") or ""), str(finding.get("root_cause_boundary") or "")))
    assert "当前不能确认" in text
    assert "电源根因已确认" not in text and "接地根因已确认" not in text and "SLIC根因已确认" not in text
