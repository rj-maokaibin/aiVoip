...
def _track(samples: np.ndarray) -> RenderedRtpTrack:
    return RenderedRtpTrack(
        ...
        samples=samples,
        packet_count=500,
        inserted_loss_samples=0,
        missing_payload_packets=0,
        sequence_first=1,
        sequence_last=500,
    )


def _correlations(score: float = 0.95) -> list[dict]:
    return [{
        ...
        "details": {
            ...
            "correlation": {"absolute_correlation": score, "quality": "HIGH" if score >= 0.8 else "MEDIUM"},
        },
    }]
