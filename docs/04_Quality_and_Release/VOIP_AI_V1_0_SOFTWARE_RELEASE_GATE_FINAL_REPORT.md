...
- `LIVE_FEISHU_TENANT` ¡ª **VERIFIED** (send acceptance PASS, 2026-08-19)
- `REAL_DUT_END_TO_END` ¡ª Pending (not performed)
- `REAL_SEMANTIC_AND_GOLDEN_DATASET` ¡ª Pending (user decision C: not accepted; synthetic golden PASS)
...
### LIVE_FEISHU_TENANT ¡ª VERIFIED (send acceptance PASS)
...
- Real send acceptance PASS (user-authorized): a test interactive card was
  ...
  (`FeishuLiveTransport.send_card` ¡ú `POST /im/v1/messages`); Feishu returned
...
real semantic/Golden dataset direction (user decision C ¡ª external Pending):
