...
docker run -d --rm --name "$PG_CONTAINER" \
  -e POSTGRES_DB=voip -e POSTGRES_USER=voip -e POSTGRES_PASSWORD=voip \
...
for _ in $(seq 1 60); do
  if docker exec "$PG_CONTAINER" pg_isready -U voip -d voip >/dev/null 2>&1; then break; fi
  sleep 1
done
...
if [[ -f backend/tests/test_ai1_semantic_router_v1.py ]]; then
  ...
  pytest -q \
    backend/tests/test_ai1_semantic_router_v1.py \
    backend/tests/test_ai1_semantic_gateway_v1.py \
    backend/tests/test_ai1_semantic_eval_gate_v1.py \
    backend/tests/test_ai1_semantic_api_v1.py \
    backend/tests/test_ai1_semantic_real_corpus_eval_v1.py
else
  log "4/11 AI1 gate skipped: implementation not present on this branch"
fi

if [[ -f backend/tests/test_ai3_case_copilot_v1.py ]]; then
  log "5/11 AI3 Case Copilot gate"
  pytest -q \
    backend/tests/test_ai3_case_copilot_v1.py \
    backend/tests/test_ai3_copilot_gateway_v1.py \
    backend/tests/test_ai3_copilot_api_v1.py \
    backend/tests/test_ai3_copilot_idempotency_isolation_v1.py \
    backend/tests/test_ai3_feishu_copilot_v1.py \
    backend/tests/test_ai3_feishu_tenant_idempotency_v1.py \
    backend/tests/test_ai3_copilot_fail_closed_v1.py
else
  log "5/11 AI3 gate skipped: implementation not present on this branch"
fi
...
if [[ -f backend/tests/test_ai2_diagnostic_loop_v1.py ]]; then
  ...
  log "6/11 AI2 gate skipped: implementation not present on this branch"
fi
...
printf '\n=============================================\n'
printf 'VOIP AI SOFTWARE RELEASE GATE: PASS\n'
